package project.euler;
import java.math.BigInteger;

/**
 * Created by shashi on 1/28/14.
 */
public class LargestProductInASeries {

   public Integer solve()
    {






        BigInteger myBigInteger = new BigInteger("");

        int factor = 5;
        Integer multiply=new Integer(0);
        int actuallength =  myBigInteger.toString().length();
        System.out.println("actuallength :" + actuallength);
        for(int n=0;n<(actuallength+1)-factor;n++)
        {
            System.out.print("n : "+ n + "::");
            int length = myBigInteger.toString().length();
            Integer temp = new Integer(1);
            BigInteger tempInteger = null;
            for(int i =0;i<factor;i++)
            {
                BigInteger remainder = null;
                if(i==0)
                {
                    int templength = length;

                    remainder = myBigInteger.divide(BigInteger.TEN.pow(length-1));
                    myBigInteger=myBigInteger.subtract(BigInteger.TEN.pow(length-1).multiply(remainder));
                    length = myBigInteger.toString().length();
                    if(templength-1>length)
                    {
                        n=n+(factor-i);
                        break;
                    }
                    tempInteger = myBigInteger;
                }
                else
                {
                    int templength = length;
                    remainder = tempInteger.divide(BigInteger.TEN.pow(length-1));
                    tempInteger=tempInteger.subtract(BigInteger.TEN.pow(length - 1).multiply(remainder));
                    length = tempInteger.toString().length();
                    if(templength-1>length)
                    {

                        n=n+(factor-i+1);
                        break;
                    }
                }

                System.out.print(remainder);
                temp = temp*remainder.intValue();

                if(temp>multiply)
                {
                    multiply = temp;
                } 

            }
            System.out.print("=" + temp);
            System.out.println();
        }

        System.out.println(multiply);
    }

   
}
